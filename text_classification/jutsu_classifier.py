...
from .custome_trainer import CustomTrainer
import gc
...
class JutsuClassifier:
    def __init__(self,
        ...
        if not huggingface_hub.repo_exists(self.model_path):
            ...
            if data_path is None:
                raise ValueError(
                    'Data path is required to train when model path is not a huggingface repo')
            ...
            all_data = pd.concat(
                [train_data_df, test_data_df]).reset_index(drop=True)
    ...
    def simplify_jutsu_type(self, jutsu):
        if jutsu == 'Genjutsu':
            return 'Genjutsu'
        if jutsu == 'Ninjutsu':
            return 'Ninjutsu'
    ...
    def load_data(self, data_path):
        ...
        df['jutsu_type_simplified'] = df['jutsu_type'].apply(
            self.simplify_jutsu_type)
        ...
        label_dict = {index: label_name for index,
                      label_name in enumerate(le.__dict__['classes_'].tolist())}
        ...
        tokenized_train = train_dataset.map(lambda examples: self.preprocess_function(
            self.tokenizer, examples), batched=True)
        tokenized_test = test_dataset.map(lambda examples: self.preprocess_function(
            self.tokenizer, examples), batched=True)
